...
@REM Write "main-2s" on "main-2.txt", and stash
echo main-2s >> main-2.txt
git stash save main-2.txt

@REM Create "main-3s.txt", write "main-3s", and stash
touch main-3s.txt
...
@REM Commit everything before checking out master
git add .
git commit -m "Add untracked files"

@REM Change main-2.txt and commit
git checkout master
echo main-2master >> main-2.txt
git add .
git commit -m "Modified main-2.txt"

@REM Apply the last stash (should cause a conflict)
